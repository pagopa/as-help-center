document.addEventListener('DOMContentLoaded', function() {
  const ssoLogoutReturnToAddressWrappper = document.querySelector(`[data-auth-sso-logout-return-to-address]`);
  ...
  if (isLogoutTimerPage) { 
    const timeoutLogoutDurationWrapper = document.querySelector(`[data-auth-timeout-logout-duration]`);
    if (!timeoutLogoutDurationWrapper) {
        return;
    }

    const timeoutLogoutDurationS = timeoutLogoutDurationWrapper.innerText ? Number(timeoutLogoutDurationWrapper.innerText.trim()) : 1; // default 1 minute
